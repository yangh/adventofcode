...
(define (op3 ints pos ID)
  (vector-set! ints
               (number-at ints (+ pos 1))
               ID))
  ...
  (when (< pos (vector-length ints))
    (let ([op (vector-ref ints pos)])
      (cond
        ...
        [(char=? (last (string->list op)) #\1)
         (op12 + ints pos)
         (intcode ints (+ pos 4) ID)]
        [(char=? (last (string->list op)) #\2)
         (op12 * ints pos)
         (intcode ints (+ pos 4) ID)]
        [(string=? op "3")
         (op3 ints pos ID)
         (intcode ints (+ pos 2) ID)]
        [(char=? (last (string->list op)) #\4)
         (op4 ints pos)
         (intcode ints (+ pos 2) ID)]
        [else
...
; Part 1
(intcode (input->vector input) 0 "1")
